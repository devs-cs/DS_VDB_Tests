...
def reconstruction(
    ...
    vec_to_avg = [proj_embeddings]
    copy_vec = proj_embeddings.squeeze(1)
    for vec in  rel_vec:
        cos_sim = F.cosine_similarity(copy_vec, vec)
        if (cos_sim > 0.5):
            vec_to_avg.append(vec)
    new_proj_embeddings = sum(vec_to_avg)/len(vec_to_avg)
    proj_embeddings = new_proj_embeddings.unsqueeze(dim = 0)
    ...
    if retrieve==False:
        ...
        v2c_reference_out = nn.functional.normalize(proj_embeddings.view(-1,768),dim=-1)
        ...
        for ii,im in enumerate(retrieved_clips):
            ...
            # print(f"currecon shape: {currecon.shape}")
